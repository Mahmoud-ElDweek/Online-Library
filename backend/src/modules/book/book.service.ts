...
export class BookService {
  ...
  addNewBook = async (book: BookDTO, file: Express.Multer.File) => {
    ...
    if (isNaN(pages) || pages < 0) {
      throw new HttpException(
        'Invalid pages count. Pages must be a valid number greater than 0.',
    ...
    if (isNaN(stock) || stock < 0) {
      throw new HttpException(
        'Invalid Stock. Stock must be a valid number greater than 0.',
        HttpStatus.BAD_REQUEST,
      );
    }
    book.pages = pages;
  ...
  updateThisBook = async (
    ...
    const updateBook = await this.bookModel
      .findByIdAndUpdate({ _id: bookId }, { $set: book }, { new: true })
      .populate('author')
      .populate('category');
  ...
  async getBooksByRecommendation(
    ...
    try {
      ...
      return {
        message: 'Error fetching the recommended books.',
        error: error.message,
      };
    }
  }

  async findByAuthorOrTitle(
    paginationDTO: PaginationDTO,
    author: string,
    title: string,
  ) {
    ...
    if (author && author.trim() !== '') {
      ...
      const authors = await this.authorModel
        .find({ name: { $regex: author.trim(), $options: 'i' } })
        .exec();
      if (authors.length > 0) {
